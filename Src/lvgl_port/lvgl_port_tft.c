...
static void flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map) {
    ...
    // Обновляем смещение вывода для текущей области
    hdma2d.Init.OutputOffset = TFT_HOR_RES - width;
    HAL_DMA2D_Init(&hdma2d);  // Нужно повторно инициализировать после изменения OutputOffset?

    // Перезапускаем слой, если нужно (можно убрать, если нет)
...
static void dma2d_init(void) {
    ...
    hdma2d.Init.OutputOffset = 0; // Установим при каждом запуске отдельно
    ...
    // Регистрация коллбеков (один раз)
